    ...
    <section className="py-24 bg-brand-gray relative overflow-hidden">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          <h2 className="text-sm font-bold text-brand-red tracking-widest uppercase mb-4">Our Creative Step Process</h2>
          <h3 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6">How We Work</h3>
        </div>

        <div className="relative">
          {/* Vertical Line */}
          <div className="absolute top-0 bottom-0 left-[27px] md:left-1/2 w-[2px] bg-white/5 -translate-x-1/2"></div>
          ...
          <div className="space-y-16 relative z-10">
            {steps.map((step, index) => (
              ...
              >
                {/* Content */}
                <div className={`flex-1 w-full md:w-1/2 ${index % 2 === 0 ? 'md:text-left' : 'md:text-right'} pl-16 md:pl-0`}>
                  <h4 className="text-2xl font-bold mb-3 text-white">{step.title}</h4>
                  <p className="text-gray-400 text-lg leading-relaxed">{step.description}</p>
                </div>

                {/* Number Node */}
                <div className="absolute left-0 md:relative md:left-auto w-14 h-14 bg-brand-black border border-white/10 rounded-full flex items-center justify-center shrink-0 z-10 shadow-xl">
                  <span className="text-brand-red font-bold text-lg">0{index + 1}</span>
                </div>
