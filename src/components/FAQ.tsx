...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="py-16 md:py-24 bg-brand-gray relative">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10 md:mb-16">
          <h2 className="text-xs sm:text-sm font-bold text-brand-red tracking-widest uppercase mb-3 md:mb-4">Frequently Asked Questions</h2>
          <h3 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 md:mb-6 leading-tight">Got Questions? We've Got Answers</h3>
          ...
          {faqs.map((faq, index) => (
            ...
            >
              <button
                ...
                className="w-full px-4 sm:px-6 py-4 sm:py-5 text-left flex justify-between items-center gap-3 hover:bg-white/5 transition-colors focus:outline-none"
              >
                <span className={`font-bold text-base sm:text-lg transition-colors text-left ${openIndex === index ? 'text-brand-red' : 'text-white'}`}>
                  {faq.question}
                </span>
                <ChevronDown
                  size={24}
              ...
              <AnimatePresence>
                {openIndex === index && (
                  ...
                  >
                    <div className="px-4 sm:px-6 pb-4 sm:pb-5 text-gray-400 text-sm sm:text-base leading-relaxed border-t border-white/5 pt-3 sm:pt-4">
                      {faq.answer}
                    </div>
                  </motion.div>
                )}
